...
class AIQuizGenerator {
  ...
  constructor() {
    this.skillScenarios = this.initializeSkillScenarios();
  }
  
  initializeSkillScenarios() {
    return {
      "SQL": {
        "beginner": [
          {
  ...
  getTimerForDifficulty(difficulty) {
    const limits = this.TIMER_LIMITS[difficulty];
    if (!limits) {
   ...
   */
  generateQuiz(skillName, difficulty) {
    // Normalize inputs
  ...
  generateQuestionByType(skillName, difficulty, questionType) {
